...
let totalCount = 0 // 总页数
async function setArtuleList() {
  const result = await axios({
    url: '/v1_0/mp/articles',
    params: queryObj
  })
  const trStr = result.data.results.map(item => {
    return `<tr>
      <td>
  ...
  document.querySelector('.align-middle').innerHTML = trStr
 ...
 */
document.querySelector('.align-middle').addEventListener('click', async e => {
  if (e.target.classList.contains('del')) {
    ...
    const trCount = document.querySelector('.align-middle').children.length
    if (trCount === 1 && queryObj.page > 1) {
      queryObj.page--
    }
    setArtuleList()
  }
